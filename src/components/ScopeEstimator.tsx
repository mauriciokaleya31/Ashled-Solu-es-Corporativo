...
export const ScopeEstimator: React.FC<ScopeEstimatorProps> = ({ onStartProject }) => {
  ...
  const getUrgencyBadge = () => {
    switch (urgency) {
      case 'urgent':
        return { label: 'Execução Imediata / Fast-Track', color: 'text-black bg-[#FE8D00]' };
      case 'priority':
        return { label: 'Prioridade Alta', color: 'text-white bg-neutral-800 border border-[#FE8D00]/50' };
      default:
        return { label: 'Cronograma Padrão', color: 'text-neutral-300 bg-neutral-800' };
    }
  };
  ...
  const getGeoBadge = () => {
    switch (geoScope) {
      case 'global':
        return 'Internacional (China / Europa / Américas)';
      case 'regional':
        return 'Regional (África Austral / SADC)';
      default:
        return 'Nacional (Angola / Luanda)';
    }
  };
    ...
    <div className="p-6 sm:p-10 rounded-3xl bg-neutral-900 border border-neutral-800 shadow-2xl space-y-8">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-neutral-800 pb-6">
        <div className="space-y-1">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#FE8D00]/20 border border-[#FE8D00] text-[#FE8D00] text-xs font-black uppercase tracking-wider">
            ...
            <span>Simulador de Âmbito & Estruturação</span>
          </div>
          <h3 className="text-2xl sm:text-3xl font-black text-white tracking-tight">
            Configure a Sua Necessidade de Serviço
          </h3>
          <p className="text-xs sm:text-sm text-neutral-400 max-w-xl">
            Selecione o serviço pretendido, abrangência geográfica e nível de urgência para visualizar o fluxo de coordenação Ashled.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="space-y-3">
            <label className="text-xs font-bold uppercase tracking-wider text-neutral-400 block">
              1. Selecione o Pilar de Atuação:
            </label>
          ...
          <div className="space-y-3">
            <label className="text-xs font-bold uppercase tracking-wider text-neutral-400 block">
              2. Âmbito Geográfico da Operação:
            </label>
            <div className="grid grid-cols-3 gap-2">
              {[
                { id: 'local', label: 'Local (Angola)' },
                { id: 'regional', label: 'Regional (África)' },
          ...
          <div className="space-y-3">
            <label className="text-xs font-bold uppercase tracking-wider text-neutral-400 block">
              3. Nível de Prioridade & Cronograma:
            </label>
            <div className="grid grid-cols-3 gap-2">
              {[
                { id: 'standard', label: 'Planeado / Padrão' },
                { id: 'priority', label: 'Prioritário (15-30 d)' },
                { id: 'urgent', label: 'Urgente (< 15 dias)' },
              ].map((u) => (
                <button
                  key={u.id}
        ...
        >
          ...
          <div className="space-y-3">
            <div>
              <div className="text-[10px] text-neutral-500 uppercase font-mono">Serviço Selecionado</div>
              <h4 className="text-lg font-black text-white">{t(currentService.titleKey)}</h4>
            </div>

            <div className="grid grid-cols-2 gap-3 text-xs">
              <div>
                <span className="text-neutral-500 block text-[10px] uppercase font-mono">Cobertura</span>
                <span className="text-white font-semibold">{getGeoBadge()}</span>
              </div>
              <div>
                <span className="text-neutral-500 block text-[10px] uppercase font-mono">Garantia</span>
                <span className="text-[#FE8D00] font-semibold">100% Coordenado</span>
              </div>
            </div>
          ...
          >
            ...
            <span>Solicitar Cotação para Esta Configuração</span>
          </button>
        </motion.div>
      </div>
    </div>
